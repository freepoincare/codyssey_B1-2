...
### 1.1 브랜드 기본 정보
- **브랜드명:** 에코 바이트 (Eco-Bite)
- **타겟 고객:** 환경 보호에 관심이 많고 배달/테이크아웃을 자주 이용하는 2030 1인 가구 및 친환경 가치 소비층.
- **톤앤매너:** 내추럴 오가닉, 생동감 넘치는 매지컬 리얼리즘(Magical Realism), 시네마틱 4K, 따뜻하고 희망찬 무드.
...
- **생성 결과 파일명:** `scene01_plastic_waste.png` / `scene01_intro_zoom.mp4` / `scene01_narration01.wav`

---

### ◼️ 씬 2 (Concept: 배달앱에서의 '에코 바이트' 선택)

<img src="assets/images/scene2_eco-friendly-order.png" width="300">
...
- **생성 결과 파일명:** `scene02_app_select.png` / `scene02_ui_touch.mp4` / `scene02_touch_sfx.wav`

_참고: 씬 2부터 GPT-image에 생성한 인물 키 비주얼을 Flow에 업로드 하는 과정에서 경고문 발생 "Our policies prohibit uploading of prominent people at this time. Please try a different image or send feedback.". 따라서 GPT-image 사용 결정._

---

### ◼️ 씬 3 (Action: 친환경 용기 도착 및 식사 후 이동)

<img src="assets/images/scene3_heading-to-balcony.png" width="300">
...
- **사용 도구 및 목적:**
  - 이미지: GPT-image / 감자 전분 질감의 빈 용기를 들고 싱그러운 정원으로 걸어나가는 주인공의 조화로운 컷 생성.
...
- **출력 결과 요약 (한 줄):** 앱 선택 결과로 도착한 친환경 용기를 들고 자연 친화적인 공간이나 화단으로 이동하는 주인공의 경쾌한 움직임 연출.
- **생성 결과 파일명:** `scene03_walking_garden.png` / `scene03_tracking_shot.mp4` / `scene03_narration02.wav`

---

### ◼️ 씬 4 (Climax: 마법 같은 생분해 모션 - 핵심 비주얼)

<img src="assets/images/scene4_flower_burst.png" width="300">
...
- **사용 도구 및 목적:**
  - 이미지: Midjourney / 부서진 용기 사이로 다채로운 꽃들이 피어오르는 경이로운 찰나의 순간 생성.
...
- **생성 결과 파일명:** `scene04_flower_burst.png` / `scene04_blooming_climax.mp4` / `scene04_magic_sfx.wav`

---

### ◼️ 씬 5 (Message: 핵심 슬로건 전달)
- **씬 길이:** 6초 (누적: 27초)
...
- **사용 도구 및 목적:**
  - 이미지: Midjourney / 풍성하고 아름다운 햇살 가득한 꽃밭 배경 소스 생성.
...
- **생성 결과 파일명:** `scene05_golden_field.png` / `scene05_wind_sway.mp4` / `scene05_main_copy.wav`

---

### ◼️ 씬 6 (Outro - 브랜드 인지 및 CTA)

- **씬 길이:** 5초 (누적: 32초)
- **목표 메시지:** 브랜드 로고와 웹사이트 주소를 노출하여 구매 및 검색 유도 (마지막 3~5초 구간 제약 준수).
- **화면 구성:** 깨끗하고 미니멀한 오가닉 그린 배경. 중앙에 새싹 모양이 결합된 'Eco-Bite' 브랜드 로고와 슬로건이 선명하게 등장. 하단에 검색창 그래픽과 CTA 문구 배치.
- **내레이션 / 카피:** (자막) "이제 용기까지 자연에서 온 '에코 바이트'로 바꿀 시간. [검색창: 에코 바이트]"
- **사용 도구 및 목적:**
  - 이미지: Midjourney / 브랜드의 정체성을 보여주는 미니멀하고 깨끗한 오가닉 그린 그라데이션 배경 생성.
...
- **생성 결과 파일명:** `scene06_outro_bg.png` / `scene06_final_logo.mp4` / `scene06_jingle.wav`

---

## 3. 최종 영상 파일 정보

- **최종 파일명:** `[브랜드명_캠페인명_팀명/이름.mp4]`
- **총 재생 시간:** 32초
...
## 4. 제작 파이프라인 및 리스크 관리 전략

### 4.1 생성 도구 파이프라인 (주 도구 및 선택 이유)
* **이미지 생성:** [도구명: Midjourney, 이 도구를 선택한 이유 기술]
* **비디오 생성/변환:** [도구명: Runway Gen-3, 이 도구를 선택한 이유 기술]
* **오디오 생성 (BGM/효과음):** [도구명: Suno / ElevenLabs, 이 도구를 선택한 이유 기술]

### 4.2 리스크 대응을 위한 대체 도구 리스트
크레딧 부족 및 대기열 이슈 발생 시 전환할 대체 도구를 미리 정의합니다.
- **이미지 생성 대안:** [예시: Adobe Firefly, Stable Diffusion 등]
- **비디오 생성 대안:** [예시: Luma Dream Machine, Pika, Kling 등]
- **오디오/음성 생성 대안:** [예시: Udio, Typecast, Vrew 등]

### 4.3 일관성(스타일/캐릭터) 유지 전략
- **적용 기술/파라미터:** [예시: Midjourney의 `--sref` 스타일 참조 및 `--cref` 캐릭터 참조 코드 고정 사용 / 모델 인물 고정을 위해 ControlNet 사용 등 구체적 방법 기재]
...
- **발생한 문제점:** 혼자서 먹었는데 나무 젓가락이 여러 개 놓여져 있는 이미지가 생성됨 (Flow, GPT-image 둘 다). 주방은 안 보였으면 함.
- **수정 후 프롬프트 (원문):** `A cinematic top-down view of a small, dimly lit apartment dining table cluttered with empty plastic takeout containers, disposable utensils, and food packaging. A single used wooden chopstick is placed directly inside the container in front of the man. No kitchen is visible. A 27-year-old Korean man sits alone at the table, staring at the pile of waste with a tired expression and a subtle sigh, conveying guilt and discomfort. The room is filled with cold, muted gray-blue tones, creating a heavy and somber atmosphere that reflects the environmental impact of everyday convenience. Realistic urban lifestyle photography, dramatic shadows, shallow depth of field, emotional storytelling, high-end commercial film still, ultra-realistic, 8K. Empty space at the bottom of the frame reserved for subtitle text. Screen with 16:9`
- **결과 변화:** 의도대로 나무 젓가락이 한 개만 나오고 주방은 보이지 않음.
