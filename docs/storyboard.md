...
- **입력 프롬프트:** 
    ...
    * `Flow - Omni Flash`, Frames, 16:9, 1x, 6s
    ...
    참고) 앱에 한글이 깨져서 나오지만 이를 개선할 방법은 프롬프트로 없다고 함. 프롬프트에 한글 글씨 그대로 나오게 해달라고 요청했지만 그래도 깨져서 나옴.
...
- **입력 프롬프트:** 
  ...
  * Start: 씬 4 이미지
  * `Flow - Omni Flash`, Frames, 16:9, 1x, 6s ➔ 생성 후 마지막 1s 삭제
...
- **입력 프롬프트:**
  ...
  * Start: 씬 5 이미지
  * End: 씬 6 이미지
  * `Flow - Veo 3.1 Lite`, Frames, 16:9, 1x ➔ 알아서 8s 생성
...
- **최종 파일명:** `[eco-bite.mp4]`
- **총 재생 시간:** 39초
- **해상도:** 1280x720 (720p) [또는 1920x1080 (1080p)]
- **프레임레이트:** 사용 안 함 [24fps / 30fps 중 선택]
- **비디오 코덱:** H.264
- **오디오 코덱:** AAC
