...
기획 및 프롬프트 상세 내용 확인: `docs/` 폴더 내의 [마크다운 기획서](docs/storyboard.md) 혹은 PDF 파일을 참조.

최종 광고 영상 재생: 최상위 디렉토리의 `eco-bite.mp4`를 플레이어로 재생하거나, [유튜브/드라이브 링크 입력]를 통해 확인 가능.

## 💡 결과 및 회고

이미지와 비디오를 내가 원하는 대로 프롬프트로 적어서 생성한다는 것이 어려운 일임을 느낌. 특히 용기가 꽃밭에서 분해되고 꽃이 자라나는 것까지 내가 원하는 그림과 비디오를 생성하기 위해서 많은 어려움을 느낌. 프롬프트를 여러 번 바꾸고 Flow에서도 Sora에서도 시도 했음. 네이토의 영상 생성은 Start와 End 이미지를 넣을 수 없기에 씬 간의 연결성이 없어서 영상을 쓰지 않음.

스토리 보드를 최대한 확정 후 비디오로 전환하려는 계획이었는데 비디오를 생성하면서 장면마다 스토리가 이어지도록 스토리 보드에 넣었으면 하는 씬들이 더 생김.
