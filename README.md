...
**무료 제한:**
* Flow: 매일 50 AI 크레딧을 제공. 무료 버전은 Start이미지만 넣을 수 있음. 비디오 생성에서 [프레임, 16:9, Omni Flash, 6s] 조합으로 10 크레딧 소모. (Runway Gen-3는 유료 버전으로 안내해서 사용 안 함)
...
* **크레딧/대기열 리스크 관리:**
  - 이미지 단계에서 스토리보드를 최대한 확정 후 비디오 변환을 진행하여 실패 비용을 최소화. 비디오는 비 메인 도구로 이미지에서 비디오로 변환 시도 후 어떻게 나오는지 확인 후에 메인 도구 사용.
...
기획 및 프롬프트 상세 내용 확인: `docs/` 폴더 내의 [마크다운 기획서](docs/storyboard.md) 혹은 [PDF](docs/storyboard.pdf) 파일을 참조.

최종 광고 영상 재생: 최상위 디렉토리의 `eco-bite.mp4`를 플레이어로 재생하거나, [유튜브](https://youtu.be/iHyX0dlCTBQ)를 통해 확인 가능.

## 💡 결과 및 회고

이번 프로젝트를 통해 이미지와 비디오를 프롬프트만으로 원하는 형태로 생성하는 과정이 예상보다 높은 난이도를 가진다는 점을 체감하였다. 특히 용기가 꽃밭에서 분해되고, 그 과정에서 꽃이 자라나는 장면과 같이 구체적인 연출 의도가 포함된 경우에는 원하는 결과물을 얻기까지 상당한 반복 작업이 필요했다. 이를 위해 프롬프트를 여러 차례 수정하며 Flow와 Sora 등 다양한 생성 도구를 활용해 테스트를 진행하였다.

도구별 특성과 한계도 확인할 수 있었다. 네이토의 영상 생성 기능은 Start 이미지와 End 이미지를 지정할 수 없어 장면 간 연결성과 서사적 흐름을 유지하는 데 제약이 있었기 때문에 이번 프로젝트에서는 네이토를 활용하지 않았다.

프로젝트 초반에는 스토리보드를 최대한 확정한 후 비디오 제작 단계로 전환할 계획이었으나 실제 영상 생성 과정에서는 예상하지 못했던 전환 장면이나 보완 장면의 필요성이 발견되었다. 그 결과, 스토리보드가 단순히 영상 제작의 선행 단계가 아니라 영상 생성과 병행하며 발전하는 산출물이라는 점을 경험했다. 특히 장면 간 스토리의 흐름을 자연스럽게 연결하기 위한 추가 씬의 필요성을 확인하면서 스토리보드를 보완하게 되었다.

이번 프로젝트를 통해 생성형 AI 기반 콘텐츠 제작에서는 초기 기획의 완성도뿐만 아니라, 생성 결과를 검토하며 스토리와 연출을 지속적으로 수정·보완하는 반복적인 작업 과정이 필수적이라는 점을 배울 수 있었다. 또한 영상 제작 과정에서 각 AI 도구의 목적과 특성, 장단점을 충분히 이해하고 적절하게 활용하는 것이 결과물의 완성도에 큰 영향을 미친다는 점을 확인할 수 있었다. 향후에는 프로젝트의 목적과 제작 단계에 맞는 도구를 전략적으로 선택하고 활용함으로써 보다 높은 품질의 결과물을 효율적으로 제작할 수 있을 것으로 기대한다.
